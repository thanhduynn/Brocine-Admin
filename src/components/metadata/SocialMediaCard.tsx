...
export default function SocialMediaCard() {
  ...
  const { companyInformation, companyAddress, companyLink, setMetadataStore, setSocialLink } = useMetadataStore();
  const [ currentSocialLink, setCurrentSocialLink ] = useState<SocialLink[]>();

  const handleOpen = () => {
    setCurrentSocialLink(companyLink);
    openModal();
  };

  const handleClose = () => {
    if (currentSocialLink) {
      setMetadataStore('companyLink', currentSocialLink);
      closeModal();
    }
  };
          ...
          <div className="flex flex-col items-center w-full gap-6 xl:flex-row">
            ...
            <div className="flex items-center order-2 gap-2 grow xl:order-3 xl:justify-end">
              {companyLink.map(({ platform, url }) => (
                ...
                >
                  <SvgIcon name={platform as keyof typeof ICON_PATHS}/>
                </a>
              ))}
            </div>
          </div>
        ...
        <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-11">
          <div className="px-2 pr-14">
            ...
            <p className="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
              Update your details to keep your landing page up-to-date.
            </p>
          </div>
                ...
                <div className="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                  {companyLink.map(({ platform, url }) => (
                    <div key={platform}>
                      ...
                      <Input name={platform} type="text" value={url} onChange={handleChangeInput} />
                    </div>
                  ))}
                </div>
              </div>
